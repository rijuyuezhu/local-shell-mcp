...
PAYLOAD_PACKAGE_PREFIX = "workgate/ui_runtime/"
...
def _platform_wheel_lock_path(repo_root: Path) -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"workgate-wheel-{repo_key}.lock"
...
def compile_opentui(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="workgate-opentui-") as temp:
        binary_dir = Path(temp) / "binary"
        env = os.environ.copy()
        env["WORKGATE_UI_BINARY_OUTDIR"] = str(binary_dir)
        env.pop("WORKGATE_UI_EMBED_RUNTIME", None)
        _run_checked(
...
def staged_payload(
    ...
    *,
    ...
    package_root = repo_root / "src" / "workgate"
    if not package_root.is_dir():
        raise PlatformWheelError("workgate package directory is missing")
...
def build_platform_wheel(
    ...
    *,
    ...
    payload_sha256 = hashlib.sha256(payload).hexdigest()
    with tempfile.TemporaryDirectory(prefix="workgate-wheel-") as temp:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a verified workgate wheel with embedded OpenTUI",
    )
    parser.add_argument(

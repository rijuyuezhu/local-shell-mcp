...
async def git_diff(cwd: str = ".", staged: bool = False, path: str | None = None, stat: bool = False) -> dict:
    ...
    if path:
        args.extend(["--", path])
    result = await run_shell(_git(*args), cwd=cwd, timeout_s=120, max_output_bytes=500_000)
    return result.model_dump()
...
async def git_clone(repo_url: str, dest: str | None = None, branch: str | None = None, cwd: str = ".") -> dict:
    ...
    if dest:
        args.append(dest)
    result = await run_shell(_git(*args), cwd=cwd, timeout_s=1800, max_output_bytes=500_000)
    return result.model_dump()
...
async def git_fetch(cwd: str = ".", remote: str = "origin", prune: bool = True) -> dict:
    ...
    if prune:
        args.append("--prune")
    args.append(remote)
    result = await run_shell(_git(*args), cwd=cwd, timeout_s=600)
    return result.model_dump()
...
async def git_pull(cwd: str = ".", ff_only: bool = True) -> dict:
    ...
    if ff_only:
        args.append("--ff-only")
    result = await run_shell(_git(*args), cwd=cwd, timeout_s=600, max_output_bytes=500_000)
    return result.model_dump()
...
async def git_commit(cwd: str, message: str, all_changes: bool = False) -> dict:
    ...
    result = await run_shell(cmd, cwd=cwd, timeout_s=120, max_output_bytes=500_000)
    return result.model_dump()
...
async def git_show(cwd: str = ".", ref: str = "HEAD", path: str | None = None) -> dict:
    ...
    result = await run_shell(_git("show", spec), cwd=cwd, timeout_s=120, max_output_bytes=500_000)
    return result.model_dump()
...
async def git_reset(cwd: str = ".", mode: str = "soft", ref: str = "HEAD") -> dict:
    ...
    result = await run_shell(_git("reset", f"--{mode}", ref), cwd=cwd, timeout_s=120)
    return result.model_dump()

...
- Prefer high-level read for file and directory context because it supports path selectors such as path:50-80 and returns line-numbered, snapshot-grounded file output. Use tree_view, list_files, glob_search, and grep_search for discovery; read_file and read_many_files remain lower-level reading tools; edit_file, multi_edit_file, and apply_patch remain the current editing tools; write_file only when creating or intentionally replacing a whole file.

...
- Treat `read` and `search` hashline output (`[path#snapshot_id]` plus `line:text` rows) as the authoritative edit grounding. Prefer `hashline_edit` when editing directly from copied hashline text. Use `edit_lines` with `snapshot_id` when you already have structured path/start/end/replacement data. Keep ranges tight, do not infer line numbers from ungrounded snippets, and re-read after each successful edit or any stale/surprising result.

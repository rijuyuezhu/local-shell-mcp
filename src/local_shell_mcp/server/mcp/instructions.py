...
- Prefer high-level read for file and directory context because it supports path selectors such as path:50-80 and returns line-numbered, snapshot-grounded file output. Use tree_view, list_files, glob_search, and grep_search for discovery; read_file and read_many_files remain lower-level reading tools; edit_lines should be preferred for line-numbered edits after read/read_file grounding; edit_file, multi_edit_file, and apply_patch remain fallback editing tools; write_file only when creating or intentionally replacing a whole file.
- Treat read_file/read_many_files numbered_content as the authoritative line map for follow-up edits. Prefer edit_lines with snapshot_id from the read result. Do not infer line numbers from unnumbered chat snippets, and re-read when content may have changed.

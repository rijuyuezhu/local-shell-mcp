...
- Start substantial workspace work with `session_start(workdir=...)` and pass the returned `session_id` to session-bound tools. The workdir is required; ask when unclear, otherwise infer it from the task, repository, or paths. Use `session_change_cwd` when the working directory changes. Follow the semantic agent workflow: `read` for file/directory context, `search(pattern, paths=...)` for content discovery, `edit_lines` for snapshot-grounded whole-line edits, and `bash` for terminal work. Workspace-affecting tools such as `list_files`, `write_file`, `delete_file_or_dir`, `apply_patch`, `secret_scan`, file-link tools, and todo tools also require `session_id`. Connector-compatible `workspace_search` and `fetch` are read-only exceptions and do not use sessions.
- Prefer `read(session_id, path)` because selectors travel with the path: `path:50`, `path:50-80`, `path:50+20`, `path:raw`, and `path:50-80:raw`. Use `tree_view` and `glob_search` for broad path discovery, and use `list_files(session_id, path)` when the listing should be rooted in the current agent session.
...
- Prefer specialized tools over shell commands for reading, searching, and editing files. Prefer `bash` for builds, tests, package managers, git inspection, scripts, and commands that genuinely need a terminal. Understand the handle relationships before choosing a mode: `session_id` identifies the agent/workspace session and is passed to session-bound tools; `bash(async_=true)` returns a `job_id` owned by that same `session_id` and is managed with `job(session_id, poll/cancel/retry=...)`; `bash(pty=true)` returns a `shell_id` for a manually managed persistent shell and is managed only with persistent-shell companion tools. Do not use `shell_id` with `job`, and do not use `session_id` where a persistent-shell companion asks for `shell_id`.
...
- Prefer `bash` over legacy shell/job/session tools. By default it runs bounded non-interactive commands in the session workdir. Use `async_=true` for tracked long-running non-interactive work that produces a `job_id` in the same agent session. Use `pty=true` for interactive shells, REPLs, servers, or commands that need later input; PTY mode returns a `shell_id`, not a `job_id`, and the `shell_id` is separate from the agent `session_id`.
...
- Remote worker control-plane work uses `remote_admin(action, args)` for invite/list/revoke/rename. For normal remote code work, create a remote agent session with `session_start(target="remote", machine=..., workdir=...)` and then use ordinary session-bound tools with the returned control-server `session_id`. Remote sessions support file context, search, grounded edits, shell/job work, file listing, whole-file writes/deletes, patch application, and heuristic scanning through the same tool names.

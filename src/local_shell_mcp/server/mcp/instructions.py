...
- Remote worker control-plane work uses `remote_admin(action, args)` for invite/list/revoke/rename. For normal remote code work, create a remote agent session with `session_start(target="remote", machine=..., workdir=...)` and then use ordinary session-bound tools with the returned control-server `session_id`. Remote sessions support file context, tree/glob discovery, search, `hashline_edit`/`edit_lines` grounded edits, bounded or async shell/job work, background `session_copy` with controller-managed progress/cancel/retry, file listing, whole-file writes/deletes, and heuristic scanning through the same tool names; remote PTY shells are not exposed.

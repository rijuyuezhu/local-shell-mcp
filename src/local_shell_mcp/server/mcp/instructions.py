...
- Remote worker control-plane work uses `remote_admin(action, args)` for invite/list/revoke/rename. For normal remote code work, create a remote agent session with `session_start(target="remote", machine=..., workdir=...)` and then use ordinary `read`, `search`, `edit_lines`, `bash`, and `job` calls with the returned control-server `session_id`. Use `remote(op="transfer")` for binary-safe movement and `remote(op="session", args={"action": ..., "shell_id": ...})` for persistent-shell companion actions on worker-side shells.

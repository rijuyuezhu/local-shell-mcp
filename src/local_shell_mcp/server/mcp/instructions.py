...
- Remote worker control-plane work uses `remote_admin(action, args)` for invite/list/revoke/rename. For the current remote facade, `remote(op="session_start")` creates a worker-side session for remote session-bound operations, while `remote(op="session", args={"action": ..., "shell_id": ...})` manages worker-side persistent shells created by remote PTY work.

...
def _job_description(_context: object) -> str:
    return """Inspect, read durable output from, stop, or retry async bash jobs owned by an explicit agent session.

Results for `bash(async_=true)` are tracked by job id under the same session_id. Exit status and bounded output remain available after completion and server restart. Reach for this companion tool only when you need to intervene: list work in the session, inspect recent output, stop stalled work, or retry a finished command. Starting work belongs in `bash`, not `job`.

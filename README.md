...
`local-shell-mcp` can read injected agent capabilities from a read-only config
directory inside the container. With the default Docker Compose layout, write the
files on the host under:
...
- Agent bridge stdio MCP server commands run inside the container, not on the host.

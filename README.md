...
`local-shell-mcp` can read injected agent capabilities from a config directory
that it treats as read-only inside the container. With the default Docker Compose
layout, write the files on the host under:
...
The default Compose workspace mount is read-write; `local-shell-mcp` does not
mutate this directory. If you want filesystem enforcement, mount the config path
read-only in your deployment.

Set `LOCAL_SHELL_MCP_AGENT_CONFIG_DIR` to override the path. `config.json` is a
normalized manifest generated by an external sync tool:
...
At MCP startup, the server scans `skills/`, probes enabled MCP servers with a
bounded timeout, registers fixed discovery tools, and registers dynamic tools for
available skills and MCP tools before ChatGPT Web performs `tools/list`.
...
- In Docker deployments, agent bridge stdio MCP server commands run inside the container; Docker-free binary deployments run them in the same environment as `local-shell-mcp`, which may be the host.

...
| Releases | Uses an independent 4.x release line, Python 3.14+, fork-owned Docker images, executables, VSIX assets, and platform wheels with embedded OpenTUI runtimes. | Uses its independent 3.x release line and currently supports Python 3.11+. | Packages, images, configuration, state, and release assets must not be mixed between the two projects. |
...
- Docker, standalone, Python, and VS Code deployment paths.

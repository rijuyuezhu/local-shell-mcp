...
| Local and remote execution | Starts either a local or remote workspace session, then reuses the same read, edit, shell, job, Todo, Audit, and transfer tools. | Normal execution tools can select a remote machine directly; separate remote administration tools manage workers. | Workgate has a more uniform tool contract, while upstream avoids a mandatory session-creation step. |

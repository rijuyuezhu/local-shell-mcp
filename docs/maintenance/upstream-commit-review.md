...
| 152 | 2026-07-14 | `557ca1b` | fix(io): preserve PTY input and dedupe lock shards | **Implemented (adapted)** | Multi-path filesystem mutations already deduplicate the fixed cross-process lock shards before acquisition, preventing shard-collision deadlocks. Persistent terminal input uses `tmux send-keys -l` rather than direct Unix PTY or ConPTY writes, so the upstream short-write loop has no corresponding fork path. |
...
| 413 | 2026-07-19 | `1b82da8` | fix(ui): prevent dashboard alert text overlap | **Not adopted** | Upstream-only Human UI dashboard layout fix; the fork does not ship that dashboard surface. |

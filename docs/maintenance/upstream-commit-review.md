...
| 46 | 2026-06-17 | `5f0de67` | Add purpose metadata to risky tool calls | **已实现（适配）** | 高风险工具支持目的说明并进入 schema/审计。 | 当前 `run_shell_command`、`run_python_code`、`start_persistent_shell`、`job_start`、`job_retry` 等工具参数包含 `purpose`/`explanation`；generated tools reference 和 tests 覆盖。 |

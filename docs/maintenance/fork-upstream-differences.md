...
- Upstream still has several real capabilities the fork does not: structured
  Playwright browser automation, the internal HTTP ticket transport for large
  remote transfers, and multilingual documentation.
...
2. **Worker service management is implemented as local CLI-only commands.**
   Retain credential-free systemd/launchd units, Windows unsupported behavior,
   typed status/log bounds, and no MCP persistence primitive.
3. **Read-only, bounded MCP audit query is implemented.** Continue preserving
   dedicated scopes, bounded preview/detail behavior, redaction, and explicit
   local/remote session isolation.
...
6. **HTTP transfer transport is implemented after measurement.** Keep it an
   internal `session_copy` route; do not expose upstream ticket tools or weaken
   the separate-capability, durable-resume, quota, and source-only boundaries.
7. **Recoverable sanitized audit payloads are implemented with bounded retention.**

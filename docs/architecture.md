...
| `composition/services.py` | Constructs the filesystem state store and authoritative tool-session store from one resolved `Settings` object, then installs their compatibility accessors. | Controller and worker compositions need the same concrete service assembly without introducing a service locator or an executor dependency. |

...
| `executors/cli.py` | Registers the explicit `server` command and selects MCP, stdio, or REST execution from one controller-runtime composition. | Server process composition belongs beside the selectable protocol executors; leaf commands remain independent. |
...
| `executors/runtime.py` | Owns controller settings, explicitly constructed shared stores, the live `RemoteManager`, controller-local `TerminalRuntime` and `HumanUiRuntime` owners, the migrated-domain-bound tool catalog, and deterministic controller cleanup. | The controller runtime is the long-lived composition owner; it starts live owners on its event loop, closes Human UI state before remote/terminal dependencies, restores compatibility bindings last, and still passes only narrow dependencies into domain services. |
| `remote_worker/runtime_composition.py` | Owns worker settings, explicitly constructed shared stores, a worker-local `TerminalRuntime`, the migrated-domain-bound dispatcher, and the worker compatibility lifecycle. | The source-only worker needs an independent long-lived graph, including its own persistent-shell/terminal state, without importing controller runtime code. |
...
| `executors/search_composition.py` | Builds the controller Search and Files services and binds them into the explicit tool catalog through the Phase 2 registry-factory seam, including the owned `RemoteManager.call` wire dependency. | Migrated domains remain independent of ambient remote-manager lookup; controller-specific assembly belongs at the executor composition edge, not inside domain operations. |
...
| `ops/files.py` | Implements contained, bounded workspace file listing, writing, editing, hashing, deletion, link-safe path behavior, and the consumer-derived `FilesConfig`; legacy direct-call wrappers reacquire ambient dependencies only through one centralized compatibility helper. | The explicit Files service, Human UI files, remote transfer, search/secret-scan operations, and compatibility callers reuse these filesystem mechanics without making the low-level helpers own controller/worker routing. |
| `ops/files_service.py` | Owns operation-level Files/Read routing from a fresh authoritative `SessionBinding`, using explicit `FilesConfig`, `ToolSessionStore`, grounding, and an optional narrow remote callable. | Controller and source-only worker compositions construct independent service instances; migrated File/Read tool paths do not reacquire application settings, session stores, or remote-manager state through ambient getters. |
...
| `ops/read.py` | Preserves the legacy high-level Read facade while delegating selector/routing behavior to `FilesService`. | Explicit controller/worker tool paths bind the service directly; compatibility callers retain the public facade without duplicating session/path/remote policy. |
...
| `ops/search/__init__.py` | Preserves the legacy Search/glob/tree facade signatures while delegating migrated content Search to explicit composition. | Public registries, compatibility callers, and non-migrated glob/tree behavior keep a stable shared owner during the vertical migration. |
| `ops/search/core.py` | Owns pure Search configuration, path-scope parsing, ripgrep record parsing, filtering, and display-window decisions. | Pure Search decisions are independently testable and contain no process execution, session lookup, or ambient dependency access. |
| `ops/search/service.py` | Owns typed Search requests, explicit local rg execution/grounding, remote wire adaptation, and per-operation session routing. | The first migrated vertical keeps domain execution behind narrow injected dependencies and typed `SessionBinding` values. |
| `ops/search/composition.py` | Builds local Search runners and Search services from resolved settings and an explicit tool-session store. | Controller and worker may compose Search independently without a module-global service locator or cached session binding. |
...
| `ui/http/live_state.py` | Owns active terminal-connection admission/tasks plus cached Human UI remote-file worker sessions and their machine-lock shards for one controller lifespan. | The controller starts this owner after `RemoteManager`; shutdown stops UI admission, cancels active UI request/WebSocket tasks, ends owned worker sessions, and finishes before remote/terminal dependencies close. HTTP facades use only a reversible compatibility binding until narrower Phase 6 injection. |
...
| `ui/http/remote_files.py` | Normalizes remote UI paths, resolves runtime-owned cached worker sessions, dispatches workspace RPCs, reads/decodes chunks, and implements remote preview/content/mutation payloads. | Worker-session ownership lives in `ui/http/live_state.py`; this module is the Human UI adapter over remote-worker workspace tools, not the worker control plane or generic remote manager. |
...
| `ui/http/terminals.py` | Keeps the stable Human UI terminal facade: HTTP routes, OAuth scope checks, local/remote persistent-shell adapters, raw-bridge adapters, runtime-owned connection admission, and compatibility exports used by OpenTUI/tests. | Active connection ownership lives in `ui/http/live_state.py`; this facade composes the protocol and WebSocket lifecycle layers while terminal backend capabilities remain in `terminal`. |

...
| `ui/session.py` | Defines purpose-isolated Human UI session tokens, a separately derived signing key, canonical audience/origin values, origin-and-port-scoped cookie names, and transport-neutral CSRF value verification. | Browser sessions inherit approved OAuth scopes without becoming MCP bearer credentials; HTTP cookie parsing and response mutation remain in `ui/http/session.py`. |
...
| `ui/http/session.py` | Exchanges approved PKCE codes or explicitly supplied OAuth bearers into persistent HttpOnly Human UI cookies, applies double-submit CSRF and exact-origin checks, clears browser sessions, and adapts cookie authentication for HTTP and WebSocket consumers. | Standard OAuth bearer issuance remains in `oauth`; this adapter prevents JavaScript token persistence while keeping session cookies confined to the Human UI route namespace. |

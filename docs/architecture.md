...
| `ui/session.py` | Defines purpose-isolated Human UI session tokens, a separately derived signing key, canonical audience/origin values, origin-and-port-scoped cookie names, browser-binding digests, and transport-neutral CSRF verification. | Browser sessions inherit approved OAuth scopes without becoming MCP bearer credentials; the signed cookie is unusable without the companion token held by the exact browser origin, while HTTP cookie parsing and response mutation remain in `ui/http/session.py`. |
...
| `ui/http/session.py` | Exchanges approved PKCE codes or explicitly supplied OAuth bearers into persistent HttpOnly Human UI cookies bound to an origin-scoped browser companion token, applies double-submit CSRF and exact-origin checks, clears browser sessions, and adapts the binding for HTTP headers and WebSocket subprotocols. | Standard OAuth bearer issuance remains in `oauth`; this adapter prevents JavaScript bearer persistence and makes host-scoped cookies insufficient for replay from a sibling port. |

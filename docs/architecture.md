...
| `jobs/runtime.py` | Retains the compatibility surface and public shell-job orchestration while managed-job operations are delegated to `jobs/managed.py`. | Existing callers and source-only workers keep a stable jobs API during the staged refactor; this facade must not depend on controller-only `tools`. |

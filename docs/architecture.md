...
| `ui/http/session.py` | Derives the browser-visible request origin from the HTTP or WebSocket target, exchanges approved PKCE codes or explicitly supplied OAuth bearers into persistent HttpOnly Human UI cookies bound to that origin and an origin-scoped companion token, applies double-submit CSRF and exact-origin checks, clears browser sessions, and adapts the binding for HTTP headers and WebSocket subprotocols. | Standard OAuth bearer issuance and configured issuer/resource identity remain in `oauth`; this adapter prevents JavaScript bearer persistence, permits a loopback UI origin to use an external OAuth issuer, and makes host-scoped cookies insufficient for replay from a sibling scheme or port. |

...
The OAuth approval page still requires the configured admin PIN. Configure `base_url` to the externally reachable OAuth issuer and resource origin before using the server through a reverse proxy or public hostname. A browser may still open the same server through a different directly reachable origin, such as `http://localhost:8765`; the callback URI, cookies, CSRF checks, and Human UI session audience bind to that browser-visible request origin rather than to `base_url`.

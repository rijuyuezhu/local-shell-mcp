...
The access token is exchanged immediately into a purpose-isolated Human UI session and is never written to browser storage. The session uses three cooperating values: a persistent `HttpOnly`, `SameSite=Strict` cookie; a separate readable cookie for double-submit CSRF; and a cryptographically random browser-binding token stored in origin-scoped `localStorage`. The signed session records only a digest of that binding token. Every cookie-authenticated HTTP request must present the token in a custom header, and browser terminal/OpenTUI WebSockets present it through a dedicated subprotocol. Browser storage is scoped by scheme, hostname, and port, so a sibling service that receives the host-scoped cookies cannot replay them without the binding token from the exact Human UI origin. Cookie names use a stable hash of the canonical browser-visible request origin, and the signed session audience includes that same origin plus `ui_path`, so different schemes or ports remain isolated even when they share one OAuth issuer.

The session cannot outlive a finite source OAuth token. When bearer expiry is disabled, the Human UI still applies its own 30-day lifetime so the cookie remains persistent without becoming unbounded. The session remains available after closing and reopening the tab or browser and is cleared by **Sign out**, which also removes the origin binding. Only the pending PKCE verifier and OAuth `state` remain in the current tab's `sessionStorage`; that pending authorization state expires after ten minutes and disappears when the tab closes. An HTTP `401` returns to authentication; an HTTP `403` preserves a valid session and displays the missing-scope detail so a deliberately reduced session does not look expired. Remote enrollment commands are handled separately: the one-time command exists only in memory and the result dialog while visible, is never written to browser storage, and is cleared when the dialog closes, the user signs out, or the page unloads. The expandable manual-token form remains available for troubleshooting and existing tokens; a supplied bearer is converted into the same bound cookie session without being persisted by JavaScript.

The OAuth approval page still requires the configured admin PIN. Configure `base_url` to the externally reachable OAuth issuer and resource origin before using the server through a reverse proxy or public hostname. The Human UI always requires the request `Host` authority to match the browser origin. When that authority also matches `base_url`, the configured scheme supplies the browser-visible origin so HTTPS remains intact behind a TLS-terminating proxy without trusting forwarded headers. A browser may still open the same server through a different directly reachable authority, such as `http://localhost:8765`; in that case the callback URI, cookies, CSRF checks, and Human UI session audience bind to the direct browser origin.
...
Forwarded headers are ignored, so a reverse proxy connected over loopback does not grant this bypass to remote users. Browser terminal WebSockets never accept the private loopback token: in OAuth mode they require an `Origin` whose canonical authority matches the WebSocket request target and whose scheme matches the resolved browser-visible origin, the origin-scoped session-binding subprotocol, and a valid Human UI cookie session carrying both `shell:read` and `shell:execute` scopes, plus `remote:use` when the selected machine is remote. This permits an external `wss` connection to arrive at the application as `ws` only when the request authority matches the configured HTTPS `base_url`.
...
The **Audit** panel reads private process-level audit state for one selected machine, using JSONL for lifecycle/index records and the bounded payload store for authorized selected details. Unlike Files and Todos, Audit data is not owned by a workspace session: the local server reads its own process log, while a remote worker serves its own log through native `query_audit` and `get_audit_entry` RPC handlers. Remote requests therefore carry no Files/Todos `session_id`, use the existing worker allowlist, require an online exact machine match, and are capped at 60 seconds. The UI never constructs remote shell commands or copies remote log files to emulate Audit access.
...
Lists require `audit:read`; selecting a remote machine additionally requires `remote:use`. Full details are authorized again based on the actual operation: file mutations require `shell:write`, shell and job activity require `shell:execute`, shared-file activity requires `file:share`, Git writes require `git:write`, and remote activity requires `remote:use`. Unknown operation types require every supported scope except `audit:full` rather than guessing a weaker permission. The browser requests retained full sanitized payloads only for a selected detail; that second fetch additionally requires `audit:full`. These checks prevent an audit-list token from using the list endpoint to retrieve command, mutation, sharing, remote-operation, or recoverable payload content.
...
Audit fields are normalized through the shared portability, byte-budget, and credential-redaction boundary before they reach either JSONL or the private content-addressed payload store. List responses expose bounded references/previews; selected details resolve retained payloads only after digest/decompression validation and `audit:full` authorization. The browser renders JSON with a dependency-free tokenizer that creates only text nodes and fixed-class spans; OpenTUI uses a terminal-native highlighter. Stored HTML, SVG, or script-like text is never interpreted as active markup. For `view_image` records, the detail endpoint removes the original inline MCP image field, validates base64, byte limits, and PNG/JPEG/GIF/WebP magic, and preserves the bounded browser preview. When OpenTUI supplies a viewport, Pillow also decodes a first-frame RGBA thumbnail with strict source-pixel and output-dimension limits. Invalid, oversized, or undecodable image payloads produce an isolated preview error while the sanitized Audit detail remains readable.

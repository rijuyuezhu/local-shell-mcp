...
The access token is exchanged immediately into a purpose-isolated Human UI session and is never written to browser storage. The session is held in a persistent `HttpOnly`, `SameSite=Strict` cookie, while a separate readable cookie supplies the double-submit CSRF value. Cookie names include a stable hash of the complete configured origin, including its port, so two local-shell-mcp instances on different ports of the same host do not overwrite each other's browser sessions. The session cannot outlive the source OAuth token, remains available after closing and reopening the tab or browser, and is cleared by **Sign out**. Only the pending PKCE verifier and OAuth `state` remain in the current tab's `sessionStorage`; that pending authorization state expires after ten minutes and disappears when the tab closes. An HTTP `401` clears the invalid session and returns to authentication; an HTTP `403` preserves the valid session and displays the missing-scope detail so a deliberately reduced session does not look expired. Remote enrollment commands are handled separately: the one-time command exists only in memory and the result dialog while visible, is never written to browser storage, and is cleared when the dialog closes, the user signs out, or the page unloads. The expandable manual-token form remains available for troubleshooting and existing tokens; a supplied bearer is converted into the same cookie session without being persisted by JavaScript.

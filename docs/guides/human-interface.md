...
- machine-isolated local and remote tmux terminal listing, creation, raw PTY streaming, xterm rendering, snapshot fallback, input, resize, command history, and termination;
...
Windows ConPTY parity and the native OpenTUI executable remain in the explicit follow-up migration queue.
...
The **Terminals** panel manages the same tmux-backed persistent shells exposed by the MCP shell tools on one selected local or remote machine. A browser can list, create, select, interact with, resize, inspect, and terminate a shell. The control server verifies the selected worker and shell first, then treats `(machine, shell_id)` as the durable identity; two machines may safely use the same tmux session name without sharing browser state.
...
`mode=auto` negotiates a connection-scoped raw PTY bridge when the selected machine supports POSIX PTYs. The first text message is a bounded `ready` control containing only the machine, shell, selected mode, and backend. In PTY mode, all terminal output after `ready` is sent as binary WebSocket frames, and xterm input is returned as binary frames. Resize, ping, and close remain ordered JSON controls. If PTY support is unavailable—for example on the current Windows implementation or an older remote worker—`auto` falls back to the existing snapshot protocol. Clients that omit `mode` retain the original snapshot-only message sequence without a new negotiation event.

On a local POSIX machine the server opens a fresh PTY and runs `tmux attach-session` inside it. That attach client is short lived and belongs only to the WebSocket connection; the underlying tmux shell remains persistent. A remote PTY uses five internal, allowlisted worker RPCs: `open_terminal_bridge`, `read_terminal_bridge`, `write_terminal_bridge`, `resize_terminal_bridge`, and `close_terminal_bridge`. The worker returns an opaque random bridge capability only to the control server. The browser never receives that capability, a worker address, worker credentials, or a direct worker WebSocket, and none of these RPCs create or carry a Files/Todos `session_id`.

Raw bridges are exclusive per `(machine, shell_id)` so two Human UI clients cannot race terminal input into the same tmux attach stream. Open and pending bridges also count against the configured Human UI terminal connection limit. Shell IDs, bridge capabilities, dimensions, read waits, base64 envelopes, byte counts, backends, and remote response identities are independently validated. Raw reads and writes are limited to 64 KiB per operation; larger browser paste events are split into ordered 64 KiB frames. POSIX writes retry partial and temporarily blocked writes until the complete bounded chunk has been delivered in order. A malformed open response with a valid capability is closed immediately; a worker bridge that loses its controller polling lease is reaped within at most five minutes.

The bundled browser terminal uses pinned `@xterm/xterm 5.5.0` and `@xterm/addon-fit 0.10.0`. The committed JavaScript and CSS are generated reproducibly from `ui-terminal/package-lock.json`, have no source map or external font dependency, ship with the corresponding license text, and include their build source in the sdist without `node_modules`. xterm provides cursor addressing, alternate-screen applications, scrollback, full keyboard input, UTF-8, 16/256/true color, and terminal resize semantics. FitAddon measures the visible pane and sends the resulting rows and columns only after protocol negotiation completes. Because xterm creates runtime style elements and dimension attributes, the page CSP permits inline styles while keeping scripts restricted to same-origin files with no `unsafe-eval`. OSC 8 hyperlink controls are consumed before xterm records them, and a no-op link handler remains as defense in depth, so terminal output cannot navigate the browser.

The snapshot fallback remains available for compatibility and unsupported platforms. The public MCP and HTTP `read_persistent_shell_output` tool continues to return plain text by default. Only the Human UI snapshot path privately requests `tmux capture-pane -e` or `preserve_ansi=true`; the bounded inert renderer strips active OSC/DCS/APC/PM/SOS and non-SGR controls, creates only text nodes and fixed-class spans, and preserves its scroll-freeze, **Jump to latest**, and pending-update behavior.

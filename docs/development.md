...
- OAuth HTTP mode is split by responsibility: models/state, URL helpers, metadata endpoints, response serialization, dynamic client registration, authorization form/code flow, and token/JWT validation. Keep `auth.oauth` as the compatibility facade for public imports until callers are migrated to focused modules.

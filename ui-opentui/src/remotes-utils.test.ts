...
describe("remote metadata", () => {
  test("formats the worker runtime version", () => {
    expect(remoteVersion(machine({ workgate_version: "3.0.4" }))).toBe("3.0.4")
    expect(remoteVersion(machine())).toBe("—")
    expect(remoteVersion(machine({ workgate_version: 304 }))).toBe("—")
  })

  test("keeps the version out of the generic system information block", () => {
    expect(remoteSystemInfo(machine({ hostname: "node", workgate_version: "3.0.4" }))).toEqual({
      hostname: "node",
    })
  })
})
